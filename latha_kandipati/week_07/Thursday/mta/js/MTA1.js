...
function planTrip(fromLine, fromStop, toLine, toStop)
{
  ...
  if ( (fromLine === toLine) && (fromStop === toStop) ){
    console.log("You are in the stop.");
    return;
  }
  if (fromLine === toLine){
    if( linesObj[fromLine].indexOf(fromStop) > linesObj[fromLine].indexOf(toStop) ){
      for(var i=(linesObj[fromLine].indexOf(fromStop)-1); i>=(linesObj[fromLine].indexOf(toStop)); i--)
      {
        tempArray.push(linesObj[fromLine][i]);
        stopsCntr++;
      }
      console.log("You must travel through the following stops on the",fromLine+"Line: "
                +tempArray.join(", ")+".");
    }
    else{
      for(var i=(linesObj[fromLine].indexOf(fromStop)+1); i<=(linesObj[fromLine].indexOf(toStop)); i++)
